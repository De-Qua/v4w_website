...
"""
Our changes to networkx library
"""
from heapq import heappush, heappop
from itertools import count
from inspect import signature

def our_dijkstra_multisource(G, sources, weight, pred=None, paths=None, cutoff=None, target=None):
    """Uses Dijkstra's algorithm to find shortest weighted paths
    DEQUA patch: the weight function can have as fourth argument the current distance
    Parameters
    ----------
    G : NetworkX graph
    ...
    c = count()
    fringe = []
    # check the number of arguments of the weight function
    num_of_inputs = len(signature(weight).parameters)
    print(f"weights has {num_of_inputs} parameters")

    for source in sources:
        if source not in G:
            raise nt.NodeNotFound(f"Source {source} not in G")
        seen[source] = 0
    ...
    while fringe:
        ...
        for u, e in G_succ[v].items():
            ...
            if u in dist:
                u_dist = dist[u]
                if vu_dist < u_dist:
                    raise ValueError("Contraddictory paths found:", "negative weights?")
...
nt.algorithms.shortest_paths.weighted._dijkstra_multisource = our_dijkstra_multisource


"""
Our functions to be used in the graph
"""
...
def weight_battelli(x,y,dic,cost=None):
    """
    Calculates the path with public transportation.
    """
    print(cost)
    return 10
